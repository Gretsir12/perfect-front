...
export function Breadcrumbs({ items }: { items: Crumb[] }) {
  ...
  const jsonLd = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: allItems.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.label, item: `https://perfecthouse.spb.ru${item.href ?? ""}` })) };
