...
export function ProductTemplate({ item }: { item: CatalogItem }) {
  return <>
    ...
    <section className="product-gallery content-width"><p className="section-kicker">Примеры работ</p><h2>Реализованные проекты</h2><div>{item.gallery.map((image, index) => <figure key={`${image.src}-${index}`}><Image src={image.src} alt={image.alt} fill sizes="(max-width: 700px) 50vw, 25vw" /></figure>)}</div></section>
    <ProcessSteps />
    <RequestSection />
  </>;
}
